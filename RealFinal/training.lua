...
function scene:create( event )
	...
	local background = display.newImage("image/training/수련배경.png")
	...
	--수련 이미지 넣기
	local trainingSheet = graphics.newImageSheet("image/training/수련스프라이트_"..imageFlag..".png", {width = 7212 / 4, height = 789, numFrames = 4})
	local sequenceData = {
		name = "trainingImage",
		sheet = trainingSheet,
		time = 1500,
	...
	--상태 바 이미지 넣기
	local gaugebar1 = display.newImage("image/training/수련_상태바_1.png")
	gaugebar1.x, gaugebar1.y = 58 + gaugebar1.contentWidth / 2, 862 + gaugebar1.contentHeight / 2
	local gaugebar2 = display.newImage("image/training/수련_상태바_2.png")
	...
	local width = 171
	for i = 1, 23 do
		gauge[i] = display.newImage("image/training/수련_게이지바.png")
	...
	local function counter ( event )
		...
		if ( count == 23 ) then
			--수련완료 이미지
			local finish = display.newImage("image/training/수련완료.png")
