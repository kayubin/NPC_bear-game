...
function scene:create( event )
	...
	--사운드
	local waterSound = audio.loadSound("sound/shower_game/Water Sound.wav")
	local bubbleSound = audio.loadSound("sound/shower_game/Bubble Sound.wav")
	local windSound = audio.loadSound("sound/shower_game/Wind Sound.wav")
	...
	function bear_Soap (self, event)
		if(event.phase == "ended") then
			if (event.other.name == "bear") then
				if (count == 0) then
					soundEffect = audio.play(bubbleSound)
				end
	...
	function bear_Water (self, event)
		if(event.phase == "ended" and waterFlag == 0 and game_mode == 1) then
			if (event.other.name == "bear") then
				soundEffect = audio.play(waterSound)
				display.remove(event.target)
	...
	function bear_Fan (self, event)
		if(event.phase == "ended" and game_mode == 2) then
			if (event.other.name == "bear") then
				if (countFan == 0) then
					soundEffect = audio.play(windSound)
