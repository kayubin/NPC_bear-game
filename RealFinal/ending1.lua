...
------- 화면 ---------------------------------------------
	...
	local scriptBar = display.newImage("image/endingImage/스크립트 바2.png",display.contentCenterX, display.contentHeight*0.9)
	local btn = display.newImage("image/endingImage/넘김버튼.png",display.contentWidth*0.95,display.contentHeight*0.9)
	local script = display.newRect(display.contentCenterX + 20, display.contentHeight*0.9,1679,76)


	local again = display.newImage("image/endingImage/처음부터.png")
	...
	function background:tap( event ) 
		...
		if (i <= EndingData[index].pageNum) then
			...
			elseif (i == 5) then
				...
				script.fill = {
					type = "image",
					filename = EndingData[index].script
				}
			end
		else 
			background.fill = {
			...
			script.alpha = 0
			btn.alpha = 0
			if(endingCode == 30 or endingCode == 34 ) then
